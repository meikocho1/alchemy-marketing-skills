...
[![Claude Code](https://img.shields.io/badge/Claude_Code-skills-d97757.svg)](https://code.claude.com/docs)
...
| **cultural-context** | The perception-by-culture lens. Establishes how a specific region or subculture reads a signal — uncertainty avoidance, high/low context, trust locus, taboos — before you send it. |
| **market-recon** | Live fieldwork. Structured web research in the local language — competitors' signals, price bands, trust sources, taboos — synthesised into a perception profile the four moves consume. |
...
論理的な打ち手はたいてい高くて遅く、競合も全員すでにやっています。心理的な打ち手はたいてい安くて速く、まだ誰も使っていません。これらのスキルは、Rory Sutherland が著書 [_Alchemy_](https://www.goodreads.com/book/show/55165511-alchemy) で示した流儀で Claude にマーケティング/プロダクトの問題を解かせます — **意味を変える・コストのかかるシグナルを送る・当たり前の逆を試す・体感する瞬間を作り直す**。
...
| **cultural-context** | 知覚を文化で読むレンズ。シグナルを送る前に、その地域・サブカルチャーがそれをどう読むか（不確実性回避、ハイ/ローコンテクスト、信頼の所在、タブー）を確立する。 |
| **market-recon** | 現地のライブ調査。現地語でのWebリサーチ（競合のシグナル・価格帯・信頼の源・タブー）を行い、4つの打ち手が使う「知覚プロファイル」に統合する。 |
...
See [CHANGELOG.md](./CHANGELOG.md) — 変更履歴は [CHANGELOG.md](./CHANGELOG.md) を参照。
